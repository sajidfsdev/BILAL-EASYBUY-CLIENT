...
const SEARCH_BY_CUSTOMER = "VENDOR";
...
const History = (props) => {
  ...
  const handleSearchValueChange = (event) => {
    ...
    if (searchType == SEARCH_BY_CUSTOMER) {
      setCopiedRequests([
        ...requests.filter(
          (elem) =>
            elem.vendorId.name.toUpperCase().search(value.toUpperCase()) > -1
        ),
      ]);
  ...
  const handleLoadData = async () => {
    ...
    try {
      const res = await axios.post(
        AppConsts.server + "/buyer/consigned/get",
        body,
        config
      );
      if (res) {
  ...
  if (screen == LOADING_SCREEN) {
    ...
    mainGUI = (
      <React.Fragment>
        <EmptyScreen message="Sorry No History Record Found" />
      </React.Fragment>
    );
        ...
        <Row className={classes.margin}>
          ...
          >
            <TableRow>
              ...
              <TableCell align="center">
                {currentRequest.vendorId.name}
              </TableCell>
              <TableCell align="center">
                {currentRequest.vendorId.contact}
              </TableCell>
            </TableRow>
          </Table>
        </Row>
