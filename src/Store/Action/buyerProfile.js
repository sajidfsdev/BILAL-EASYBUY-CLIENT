import * as Constants from "./../Constants/buyerProfile";
  ...
  return async (dispatch) => {
    try {
      const res = await axios.post(
        AppConsts.server + "/buyer/auth/get",
        body,
        config
      );
...
export const handleEditProfileData = (profile, token) => {
  ...
  return async (dispatch) => {
    try {
      const res = await axios.post(
        AppConsts.server + "/buyer/auth/edit",
        body,
        config
      );
...
export const handleEditPassword = (oldPassword, password, token) => {
  ...
  return async (dispatch, getState) => {
    try {
      const res = await axios.post(
        AppConsts.server + "/buyer/auth/editPassword",
        body,
        config
      );
      ...
      if (res) {
        dispatch({
          type: Constants.LOADED_SUCCESS,
          payload: {
            profile: { ...getState().buyerProfile.profile },
          },
        });
        dispatch({
          type: Constants.EDITING_SUCCESS,
